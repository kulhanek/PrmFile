...
CPrmSection::CPrmSection(const CSmallString& name)
{
    Name = name;
    Next = NULL;
    FirstLine = NULL;
...
CPrmSection::~CPrmSection(void)
{
    CPrmLine* p_line = FirstLine;

    while( p_line != NULL ) {
        CPrmLine *p_next = p_line->Next;
        delete p_line;
        p_line = p_next;
    }
}
...
const CSmallString& CPrmSection::GetName(void)
{
    return(Name);
}
...
int CPrmSection::CountULines(void)
{
    int cnt = 0;
...
int CPrmSection::CountSection(void)
{
    int cnt = 0;
