...
public:
    ...
    //! was line already processed
    bool IsProcessed(void);

    //! get line data
    const CSmallString& GetText(void);

    //! split line into keyword and data
    bool Split(CSmallString& lkey,CSmallString& lvalue);

// section of private data ----------------------------------------------------
private:
    CSmallString    Text;           // Line data
    CPrmLine*       Next;           // next line
    bool            Processed;      // line was processed

    friend class CPrmFile;
    friend class CPrmGroup;
    friend class CPrmSection;
};
